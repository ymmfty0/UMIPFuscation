#include <cstdio>
#include <cstdlib>
#include <vector>
#include <Windows.h>


std::vector<unsigned char> calc_bin = {
  
};


char* GenerateUUid(int a, int b, int c, int d, int e, int f, int g, int h, int i, int j, int k, int l, int m, int n, int o, int p) {


	char* result = (char*)malloc(40);
	snprintf(result, 40, "%0.2X%0.2X%0.2X%0.2X-%0.2X%0.2X-%0.2X%0.2X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X", d, c, b, a, f, e, h, g, i, j, k, l, m, n, o, p);

	return (char*)result;
}

BOOL GenerateUUidOutput(std::vector<unsigned char>& pShellcode, SIZE_T ShellcodeSize) {

	if (pShellcode.empty() || ShellcodeSize == 0) {
		return FALSE;
	}

	while (ShellcodeSize % 16 != 0) {
		ShellcodeSize += 1;
		pShellcode.push_back(0x90);
	}

	printf("char* UuidArray[%d] = { \n\t", (int)(ShellcodeSize / 16));
	int c = 16, counter = 0;
	char* UUID = NULL;

	for (size_t i = 0; i < ShellcodeSize; ++i) {

		if (c == 16) {
			counter++;
			UUID = GenerateUUid(
				pShellcode[i], pShellcode[i + 1], pShellcode[i + 2], pShellcode[i + 3],
				pShellcode[i + 4], pShellcode[i + 5], pShellcode[i + 6], pShellcode[i + 7],
				pShellcode[i + 8], pShellcode[i + 9], pShellcode[i + 10], pShellcode[i + 11],
				pShellcode[i + 12], pShellcode[i + 13], pShellcode[i + 14], pShellcode[i + 15]
			);
			if (i == ShellcodeSize - 16) {

				printf("\"%s\"", UUID);
				free(UUID);
				break;
			}

			printf("\"%s\", ", UUID);

			c = 1;

			// Optional: To beautify the output on the console
			if (counter % 3 == 0) {
				printf("\n\t");
			}

			free(UUID);

		}
		else {
			c++;
		}
	}

	printf("\n};\n\n");
	return TRUE;
}

int main()
{
	GenerateUUidOutput(calc_bin, calc_bin.size());
}
