#include <cstdio>
#include <cstdlib>
#include <vector>
#include <Windows.h>

std::vector<unsigned char> calc_bin = {
  
};


char* GenerateMAC(int a, int b, int c, int d, int e, int f) {

	char* result = (char*)malloc(24);
	snprintf(result, 24, "%0.2X-%0.2X-%0.2X-%0.2X-%0.2X-%0.2X", a, b, c, d, e, f);

	return (char*)result;
}

BOOL GenerateMacOutput(std::vector<unsigned char>& pShellcode, SIZE_T ShellcodeSize) {

	if (pShellcode.empty() || ShellcodeSize == 0) {
		return FALSE;
	}

	while (ShellcodeSize % 6 != 0) {
		ShellcodeSize += 1;
		pShellcode.push_back(0x90);
	}

	printf("const char* MacArray [%d] = {\n\t", (int)(ShellcodeSize / 6));


	int c = 6, counter = 0;
	char* Mac = NULL;

	for (size_t i = 0; i < ShellcodeSize; i++) {

		if (c == 6) {
			counter++;

			Mac = GenerateMAC(pShellcode[i], pShellcode[i + 1], pShellcode[i + 2], pShellcode[i + 3], pShellcode[i + 4], pShellcode[i + 5]);

			if (i == ShellcodeSize - 6) {
				printf("\"%s\"", Mac);
				free(Mac);
				break;
			}

			printf("\"%s\", ", Mac);

			c = 1;

			if (counter % 6 == 0) {
				printf("\n\t");
			}

			free(Mac);

		}
		else {
			c++;
		}
	}
	printf("\n};\n\n");
	return TRUE;
}

int main()
{
	GenerateMacOutput(calc_bin, calc_bin.size());
}
